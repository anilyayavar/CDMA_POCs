...
V(dummy_gr)$group <- case_when(
  names(V(dummy_gr)) %in% dummy_data$Email ~ "E",
  names(V(dummy_gr)) %in% dummy_data$Mobile ~ "M",
  names(V(dummy_gr)) %in% dummy_data$PAN ~ "B",
  TRUE ~ "A"
)
...
make_sub <- function(g, mem, id_col) {
  ...
  visIgraph(g1, layout = "layout_with_fr") %>%
    visGroups(
      groupname = "A", shape = "icon",
      icon = list(code = "f007", size = 75, color = "red")
    ) %>%
    visGroups(
      groupname = "B", shape = "icon",
    ...
    fluidRow(
      column(
        ...
        p("In the above we can see that there are four duplicates on the basis of Mobile,
             ...
             on the basis of PAN numbers. If we draw a network chart of these 10 beneficiaries
    ...
    navlistPanel(
      ...
      tabPanel(
        ...
        fluidRow(
          ...
          column(
            ...
            fluidRow(wellPanel(
              p(em("Top 6 Rows - At most first 5 Columns
        ...
        fluidRow(
          column(
            ...
            wellPanel(
              ...
              p("Check that your ID column/variable should not have
                                              any duplicate values therein"),
              p(strong("Note: If you do not have any column that
      ...
      tabPanel(
        ...
        fluidRow(
          ...
          column(
            7,
            p(em("View First Five columns and Six rows of new data"),
              align = "center"
            ),
            hr(),
            tableOutput("head_id") # ,
...
server <- function(input, output, session) {
  ...
  output$plot_dummy <- visNetwork::renderVisNetwork(
    visIgraph(dummy_gr, layout = "layout_with_fr") %>%
      ...
      visGroups(
        groupname = "M", shape = "icon",
        icon = list(code = "f10b", size = 75, color = "#8294C4")
      ) %>%
      visGroups(
        groupname = "E", shape = "icon",
        icon = list(code = "f0e0", size = 75, color = "#93C6E7")
      ) %>%
      addFontAwesome()
  )
